...
fn generate_provider_metadata(baseurl: &Url) -> CoreProviderMetadata {
    ...
    let auth_url = AuthUrl::from_url(baseurl.join("auth/authorize").unwrap());
    let jwk_url = JsonWebKeySetUrl::from_url(baseurl.join("auth/jwk").unwrap());
    let token_url = TokenUrl::from_url(baseurl.join("auth/token").unwrap());
    let user_info_url = UserInfoUrl::from_url(baseurl.join("auth/userinfo").unwrap());
